...
$operands = [
    ...
    Operand::create('url', Operand::REQUIRED)
        ->setDescription('Url to location where package can be downloaded.')
...
$generator = new PluginDetailsGenerator(
    new File($parser->operand('source-file')),
    new Url($parser->operand('url'))
);
file_put_contents($output, $generator->json($parser->option('pretty-print')));
